...
/**
  * Allows for jumping. Tweaked to feel right, not to be realistic.
  */
class JumpBehaviour(val args: List[String]) extends Behaviour {
	val Speed = args(1).toInt.toFloat //29.0f * 2
	val InitialSpeed = args(0).toInt.toFloat //60.0f //88.55f
	// val TerminalSpeed = 20.0f * 2
	val EndTime = 0.18f
	val AdjustedSpeed = Speed / EndTime

	val airSpeedX = 80.0f // TODO: get this from physicsComponent
	...
	override def fixedUpdate(entity: Entity, scene: Scene, delta: Float): Changes = {
		...
		if (!(entity.timers.contains("recoil") && entity.timers("recoil").isRunning)) {
